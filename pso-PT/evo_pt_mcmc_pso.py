...
class Replica(PSO, Process, Particle):
    def __init__(self, num_samples, burn_in, population_size, topology, train_data, test_data, directory, temperature, swap_sample, parameter_queue, problem_type,  main_process, event, active_chains, num_accepted, swap_interval, max_limit=(10), min_limit=-10):
        ...
        PSO.__init__(self, pop_size=population_size, num_params=self.w_size, max_limits=self.max_limits, min_limits=self.min_limits,fitness_function=self.neural_network.evaluate_fitness,problem_type=self.problem_type)
        

   

    def initialize_sampling_parameters(self):
        self.weights_stepsize = 0.0005
        self.eta_stepsize = 0.0005
    ...
    def run(self):    
        ...
        if self.problem_type=='regression':
            ...
            prediction_train = self.neural_network.generate_output_classification(self.train_data, weights_current)
            prediction_test = self.neural_network.generate_output_classification(self.test_data, weights_current)

        eta = np.log(np.var(prediction_train - y_train))
        tau=np.exp(eta)
        tau_proposal = np.exp(eta)
        prior_current = self.prior_function(weights_current, tau)
        #print(np.shape(weights_current))
        [likelihood_current, rmse_train,accuracy_train] = self.likelihood_function(self.neural_network, self.train_data, weights_current, tau,self.temperature)
        rmse_test,accuracy_test=self.neural_network.classification_perf(weights_current, 'test',opt.problem_type)
        ...
        while(sample<self.num_samples):
            ...
            #print(swarm_current[0])
            
            self.swarm, self.best_swarm_pos, self.best_swarm_err = self.evolve(self.swarm, self.best_swarm_pos, self.best_swarm_err)


            for i in range(self.pop_size):
                #weights_current=curr_swarm[i].position
                
                weights_proposal=self.swarm[i].position+np.random.normal(0, self.weights_stepsize, self.w_size)
                ...
                #[likelihood_current, rmse_train_current,accuracy_train_current] = self.likelihood_function(self.neural_network, self.train_data, weights_current, tau_proposal,self.temperature)
                #rmse_test_current,accuracy_test_current=self.neural_network.classification_perf(weights_current, 'test',opt.problem_type)
                #prior_current = self.prior_function(weights_current, tau_proposal)

                accept, rmse_train, rmse_test,accuracy_train,accuracy_test, likelihood_current, prior_current = self.evaluate_proposal(self.neural_network, self.train_data, self.test_data, weights_proposal, tau_proposal, likelihood_current, prior_current)
                
            
                if accept:
                    num_accept += 1
                    weights_current = weights_proposal
                    self.swarm[i].position=weights_current
                    ...
                    if self.problem_type=='classification':
                        accuracy_train_current = accuracy_train
                        accurcy_test_current = accuracy_test
                ...
                if save_knowledge:
                    ...
                    if(opt.problem_type== 'classification'):
                        np.savetxt(train_acc_file, [accuracy_train_current])
                        np.savetxt(test_acc_file, [accuracy_test_current])
                
                #accuracy_test_check,_=self.neural_network.classification_perf(weights_current,'test')
                if(opt.problem_type=='regression'):
                    print(f'Temperature: {self.temperature:.2f} Sample: {sample} Next swap at: {self.swap_sample.value:.2f} RMSE Train: {rmse_train_current:.4f}  RMSE test: {rmse_test_current:.4f} ')
                else:
                    print(f'Temperature: {self.temperature:.2f} Sample: {sample} Next swap at: {self.swap_sample.value:.2f} accuracy train: {accuracy_train_current:.2f}  accuracy test: {accuracy_test_current:.2f} ')
                ...
                if (sample == self.swap_sample.value):
                    ...
                    #position_i=self.swarm[i].position.reshape(-1,1)
                    #print(np.shape(np.asarray([likelihood])))
                    [likelihood_particle,_,_]=self.likelihood_function(self.neural_network, self.train_data, self.swarm[i].position,np.exp(eta),self.temperature)
                    prior_particle = self.prior_function(self.swarm[i].position, np.exp(eta))
                    param = np.concatenate([self.swarm[i].position, np.asarray([eta]).reshape(1), np.asarray([likelihood_particle]),np.asarray([self.temperature]),np.asarray([prior_particle]).reshape(1)])
                ...
                #     # Wait for signal from Master
                    ...
                    self.swarm[i].position=result[:self.w_size]
...
class EvoPT(object):
    ...
    def swap_procedure(self, parameter_queue_1, parameter_queue_2):
        if not parameter_queue_2.empty() and not parameter_queue_1.empty():
            ...
            prior_1=param_1[self.num_param+3]
            ...
            prior_2 = param_2[self.num_param+3]
